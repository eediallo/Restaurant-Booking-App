...
export const BookingHistory: React.FC<BookingHistoryProps> = ({ onClose }) => {
  ...
  const isUpcoming = (booking: Booking) => {
    const bookingDateTime = new Date(`${booking.visit_date}T${booking.visit_time}`);
        ...
        <div className="text-center py-12">
          <div className="text-gray-500 mb-4">
            <svg className="w-12 h-12 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3a4 4 0 118 0v4m-4 8v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <p className="text-gray-600 mb-2">No bookings found</p>
          <p className="text-gray-500 text-sm">
            {statusFilter ? "Try adjusting your filters" : "Make your first booking to see it here"}
          </p>
        </div>
      ) : (
          ...
          {data.bookings.map((booking) => (
            <div
              ...
              className={`border rounded-lg p-4 ${
                isUpcoming(booking) ? "border-blue-200 bg-blue-50" : "border-gray-200"
              }`}
            >
                ...
                <div className="ml-4 flex flex-col gap-2">
                  {isUpcoming(booking) && (
                    <>
                      ...
                      <Button variant="outline" className="text-xs text-red-600 border-red-300 hover:bg-red-50">
                        Cancel
                      </Button>
                    </>
                  )}
